...
import { snapppayProvider } from "@/lib/payments/snapppay";

/**
 * SnappPay callback — `${APP_BASE_URL}/api/payments/snapppay/callback?orderId=...`
 *
 * SnappPay redirects the user back after the installment flow completes. The
 * gateway may return parameters via the query string (GET) or a form POST
 * (state / status, transactionId, paymentToken). We resolve the order's
 * SNAPPPAY payment, run verify (which verifies + settles), then confirm/fail
 * and funnel the user to `/payment/result?orderId=…&status=success|failed`.
 *
...
async function handle(req: NextRequest, params: Record<string, unknown>) {
  ...
  try {
    ...
    const result = await snapppayProvider.verify(payment, params);
    ...
    if (result.status === "PAID") {
      try {
        ...
        console.error(`[snapppay/callback] CAPTURED-BUT-UNCONFIRMED order ${orderId}`, confirmErr);
        await db
          .update(payments)
    ...
    console.error("[snapppay/callback] error:", err);
  }
